...
// Home video overlay — click to expand, Esc / backdrop to close
(function () {
  const corner = document.getElementById('homeCorner');
  const overlay = document.getElementById('homeOverlay');
  const backdrop = document.getElementById('homeOverlayBackdrop');
  if (!corner || !overlay || !backdrop) return;

  const video = overlay.querySelector('.home-overlay-video');
  const closeBtn = overlay.querySelector('.home-overlay-close');

  function open() {
    if (corner.classList.contains('is-expanded')) return;
    corner.classList.add('is-expanded');
  ...
  function close() {
    if (!corner.classList.contains('is-expanded')) return;
    corner.classList.remove('is-expanded');
  ...
  overlay.addEventListener('click', (e) => {
    if (corner.classList.contains('is-expanded')) return;
    if (e.target.closest('.home-overlay-close')) return;
    open();
  });
